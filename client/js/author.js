// 作者页文件

// ----- 数据交互 -----

// 分区名
const plateArr = [
  "新闻",
  "娱乐",
  "历史",
  "体育",
  "金融",
  "旅游",
  "影视",
  "美食",
];
// 分区样式
const styleArr = [
  "news",
  "game",
  "history",
  "sport",
  "finance",
  "tourism",
  "movie",
  "delicacy",
];

// 判断登录状态
const token = localStorage.getItem("token");
const uinfo = document.querySelector(".right");
const none = document.querySelector(".none");
if (token) {
  // 登录时
  uinfo.style.display = "flex";
  none.style.display = "none";
  // 渲染首页用户信息栏中的用户信息
  axios({
    method: "GET",
    url: "my/userinfo",
  }).then((res) => {
    const nickname = res.data.nickname || "游客"; // 昵称
    const user_id = res.data.id; // 用户ID
    let user_pic = ""; // 头像
    if (res.data.user_pic) {
      user_pic = "../node" + res.data.user_pic;
    } else {
      user_pic = "../node/public/upload/face.png";
    }
    document.querySelector(".nickname").innerHTML = nickname;
    document.querySelector(".nickname").dataset.id = user_id;
    document.querySelector(".face img").src = user_pic;
  });
} else {
  // 未登录时
  uinfo.style.display = "none";
  none.style.display = "block";
}

const userinfo = document.querySelector(".userinfo");
const face = document.querySelector(".face-img img");
const nick = document.querySelector(".userinfo .nickname");
const btn = userinfo.querySelector(".btn");
// 文章信息模块
(function () {
  const paramsStr = location.search; // 获取传过来的用户ID
  const params = new URLSearchParams(paramsStr); // 地址参数处理函数(内置函数)
  if (params.size < 1) return myAlert("信息获取失败"); // 判断是否接收到用户的ID
  params.forEach(async (value, key) => {
    if (key === "id") {
      try {
        // 1.获取用户信息
        const result = await axios({
          method: "GET",
          url: "/my/userinfo/id",
          params: { id: value },
        });
        let user_pic = ""; // 头像
        const nickname = result.data.nickname || "游客"; // 昵称
        userinfo.dataset.id = result.data.id; // 作者ID
        if (result.data.user_pic) {
          user_pic = "../node" + result.data.user_pic;
        } else {
          user_pic = "../node/public/upload/face.png";
        }
        nick.innerHTML = nickname;
        face.src = user_pic;
        // 2.获取文章信息
        const articleList = document.querySelector(".article-list");
        const articleFun = async () => {
          const result = await axios({
            method: "GET",
            url: "/my/article/articles/id",
            params: { uid: value, cate_id: "%", page: 1, per_page: 40 },
          });
          const htmlStr = result.data
            .map((item) => {
              return `<li data-id="${item.id}">
                <a href="javascript:;">
                  <div class="img-box">
                    <img src="../node/${item.cover_img}" alt="" />
                  </div>
                  <div class="content">
                    <div class="title">${item.title}</div>
                    <div class="tag ${styleArr[item.cate_id - 1]}">${
                plateArr[item.cate_id - 1]
              }</div>
                    <p class="time">${item.pub_date.substring(0, 10)}</p>
                  </div>
                </a>
              </li>`;
            })
            .join("");
          articleList.innerHTML = htmlStr;
          // 若没有相关文章
          if (result.data.length === 0) {
            document.querySelector(".tip").style.display = "block"; // 显示提示信息
          } else {
            // 若有相关文章
            document.querySelector(".tip").style.display = "none"; // 隐藏提示信息
          }
        };
        articleFun();
        // 3.判断关注状态
        if (token) {
          const followStatus = await axios({
            method: "GET",
            url: `/my/data/selFollow/${userinfo.dataset.id}`,
          });
          if (followStatus.data[0].count > 0) {
            btn.innerHTML = "已关注";
            btn.classList.add("follow");
            btn.dataset.code = 1;
          }
        }
      } catch (err) {
        // console.dir(err);
        return myAlert("信息获取失败");
      }
    }
  });
})();

// 关注/取消关注模块
let flag = true; // 节流阀
btn.addEventListener("click", async () => {
  // 0.判断登录状态
  if (!token) {
    alert("请先登录");
    location.href = "login.html"; // 跳转到登录页
    return;
  }
  // 1.判断是否关注自己
  const uid = document.querySelector(".top .nickname").dataset.id;
  if (uid === userinfo.dataset.id) return myAlert("不能关注自己");
  // 2.判断节流阀状态
  if (!flag) return myAlert("操作频繁,请稍等");
  flag = false; // 通过节流阀后关闭节流阀
  setTimeout(() => {
    flag = true; // 2秒后打开节流阀
  }, 2000);
  const result = await axios({
    method: "POST",
    url: `/my/data/update/follow`,
    data: { id: userinfo.dataset.id, code: btn.dataset.code },
  });
  if (result.status !== 0) return myAlert("操作失败");
  if (btn.dataset.code === "0") {
    btn.classList.add("follow");
    btn.dataset.code = "1";
    btn.innerHTML = "已关注";
    myAlert("关注成功");
  } else if (btn.dataset.code === "1") {
    btn.classList.remove("follow");
    btn.dataset.code = "0";
    btn.innerHTML = "关注";
    myAlert("取消关注");
  }
});

// 退出登录模块
const exit = document.querySelector(".exit");
exit.addEventListener("click", () => {
  // 1.清空本地保存的令牌
  localStorage.removeItem("token");
  // 2.跳转到登录页
  myAlert("退出成功");
  setTimeout(() => {
    location.href = "login.html"; // 跳转到登录页
  }, 1000);
});

// ----- 网页交互 -----

// 用户信息栏交互模块
const user = document.querySelector(".right");
const panel = document.querySelector(".panel");
user.addEventListener("mouseenter", () => {
  panel.classList.add("show");
});
user.addEventListener("mouseleave", () => {
  panel.classList.remove("show");
});

// 文章交互
// 重点 渲染+委托+查找触发元素
// 1.由于文章是后面渲染的,所以只能采用事件委托来添加交互效果
// 2.通过监听父元素article-list来给子元素li添加交互效果
// 3.但由于li中还有其他子元素,导致li被自己的子元素遮盖,无法触发
// 4.所以要对触发的子元素进行判断是不是li,不是就查找它父元素是不是li(循环查找)
const cardList = document.querySelector(".article-list");
cardList.addEventListener("mouseover", (e) => {
  let eve = e.target; // 获取被触发的子元素
  // 循环判断被触发的子元素是不是li
  while (eve.nodeName !== "LI") {
    // 如果所有父元素都不是li则跳出循环,防止死循环
    if (eve.nodeName === e.currentTarget.nodeName) break; // e.currentTarget 冒泡对象
    eve = eve.parentNode; // 不是li就向上查找,直到找到就自动退出while循环
  }
  if (eve.nodeName === "LI") {
    // 找到li后就设置交互效果
    eve.querySelector("img").style.transform = "scale(1.1)"; // 图片缩放
    eve.classList.add("li-hover"); // 阴影
  }
});
cardList.addEventListener("mouseout", (e) => {
  let eve = e.target;
  while (eve.nodeName !== "LI") {
    if (eve.nodeName === e.currentTarget.nodeName) break;
    eve = eve.parentNode;
  }
  if (eve.nodeName === "LI") {
    eve.querySelector("img").style.transform = ""; // 图片缩放
    eve.classList.remove("li-hover"); // 阴影
  }
});

// 文章点击跳转
cardList.addEventListener("click", (e) => {
  let eve = e.target;
  while (eve.nodeName !== "LI") {
    if (eve.nodeName === e.currentTarget.nodeName) break;
    eve = eve.parentNode;
  }
  if (eve.nodeName === "LI") {
    // 跳转到文章页并传入文章ID
    window.open(`article.html?id=${eve.dataset.id}`, "_blank");
  }
});
